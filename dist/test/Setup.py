...
setup(
    name='Loader',
    console=['loader.py'], # 'windows' means it's a GUI, 'console' It's a console program, 'service' a Windows' service, 'com_server' is for a COM server
    ...
    options={ # This is the list of options each module has, for example py2exe, but for example, PyQt or django could also contain specific options
        'py2exe': {
            ...
            'includes':['os', 'logging', 'yaml', 'flask', 'sqlalchemy', 'gevent', 'configparser', 'wtforms', 'functools', 'serial'], # All the modules you need to be included,  because py2exe guesses which modules are being used by the file we want to compile, but not the imports
        }
    },
